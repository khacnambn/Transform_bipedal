...
class SimpleWalkClient:
    ...
    def walk_forward(self, num_steps: int = 3, hip_deg: float = 100, knee_deg: float = 150, foot_deg: float = 90):
        """
    ...
    def test_angles(self):
        ...
        while True:
            try:
                user_input = input("\n> Format: joint_id angle (e.g., 1 100) or 'back': ").strip().lower()
                ...
                print(f"  Home: {config['home']} ticks = {self.ticks_to_degree(servo_id, config['home']):.2f}°")
            ...
            if user_input == "exit" or user_input == "quit":
                ...
                if len(parts) > 1:
                    try:
                        ...
                        print("❌ Invalid format. Use: walk [steps] [hip] [knee] [foot]")
                else:
                    client.walk_forward()
