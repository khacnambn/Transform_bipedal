...
class SimpleIMUFusion:
    ...
    def transform_quat_to_baselink_debug(self, q_imu, imu_name):
        ...
        print(f"     q_rot (axis-angle): w={q_rot[0]:+.4f} x={q_rot[1]:+.4f} y={q_rot[2]:+.4f} z={q_rot[3]:+.4f}")
        print(f"     q_imu (raw):        w={q_imu[0]:+.4f} x={q_imu[1]:+.4f} y={q_imu[2]:+.4f} z={q_imu[3]:+.4f}")
        
        # Step 1: q_rot * q_imu
        q_temp = self.quat_mult(q_rot, q_imu)
        print(f"     [Step 1] q_rot * q_imu:      w={q_temp[0]:+.4f} x={q_temp[1]:+.4f} y={q_temp[2]:+.4f} z={q_temp[3]:+.4f}")
        
        # Step 2: * q_rot_conjugate
        q_rot_conj = self.quat_conj(q_rot)
        print(f"     [Step 2] q_rot_conj:         w={q_rot_conj[0]:+.4f} x={q_rot_conj[1]:+.4f} y={q_rot_conj[2]:+.4f} z={q_rot_conj[3]:+.4f}")
        ...
        print(f"     ⚠️  Z change: {q_imu[3]:+.4f} → {q_baselink[3]:+.4f} (Δ={delta_z:+.4f})")
    ...
    def run(self):
        ...
        try:
            ...
            while True:
                ...
                print(f"     LEFT:  ||q|| = {norm_left_after:.6f}", "✅" if abs(norm_left_after - 1.0) < 0.01 else "🔴 BAD!")
                print(f"     RIGHT: ||q|| = {norm_right_after:.6f}", "✅" if abs(norm_right_after - 1.0) < 0.01 else "🔴 BAD!")
                ...
                # In kết quả
                print("\n📍 VỊ TRÍ (Baselink):")
                print(f"  LEFT:   X={left_p[0]:+.6f}  Y={left_p[1]:+.6f}  Z={left_p[2]:+.6f}")
                print(f"  RIGHT:  X={right_p[0]:+.6f}  Y={right_p[1]:+.6f}  Z={right_p[2]:+.6f}")
                print(f"  FUSION: X={fused_p[0]:+.6f}  Y={fused_p[1]:+.6f}  Z={fused_p[2]:+.6f}")
                ...
                print(f"  LEFT:   Roll={left_euler[0]:+7.2f}°  Pitch={left_euler[1]:+7.2f}°  Yaw={left_euler[2]:+7.2f}°")
                print(f"  RIGHT:  Roll={right_euler[0]:+7.2f}°  Pitch={right_euler[1]:+7.2f}°  Yaw={right_euler[2]:+7.2f}°")
                print(f"  FUSION: Roll={fused_euler[0]:+7.2f}°  Pitch={fused_euler[1]:+7.2f}°  Yaw={fused_euler[2]:+7.2f}°")
                
                print("\n📊 QUATERNION (Baselink):")
                print(f"  LEFT:   w={left_q[0]:+.4f}  x={left_q[1]:+.4f}  y={left_q[2]:+.4f}  z={left_q[3]:+.4f}")
                print(f"  RIGHT:  w={right_q[0]:+.4f}  x={right_q[1]:+.4f}  y={right_q[2]:+.4f}  z={right_q[3]:+.4f}")
                print(f"  FUSION: w={fused_q[0]:+.4f}  x={fused_q[1]:+.4f}  y={fused_q[2]:+.4f}  z={fused_q[3]:+.4f}")
