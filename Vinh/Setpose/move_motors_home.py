# FILE NÀY DÙNG ĐỂ ĐỌC CALIB VÀ ĐƯA SERVO VỀ VỊ TRÍ HOME (2048)
...
# calibration map góc từ tick của encoder sang góc thực tế (degree) của servo, dùng để đưa servo về vị trí home đúng góc
# file normalized là biến đổi từ tick encoder sang degree, home sẽ được ép về 0 độ
# file calib chỉ chứa home, calib , min max - dựa vào tick và độ
def load_calibration(
    calib_file: str = "bipedal_nam/config/calibration/rightcalib_normalized.json",
):
    ...
    try:
        ...
        motor_info = data.get(
            "motor_info", {}
        )  # lấy data của từng servo - id, homestep, min max step
        ...
        for motor_id_str, info in motor_info.items():
            ...
            home_positions[motor_id] = info.get(
                "home_steps", 2048
            )  # lôi giá trị homestep ra, nếu không ghi gì thì mặc định là 2048 (số dự phòng)

            # Store min/max in ticks (linear 0-4095)
            # CẦN CHECK LẠI PHẦN NÀY, XEM XEM CÓ ĐÚNG THẬT LÀ TẤT CẢ SERVO HOME ĐỀU LÀ 2048 KHÔNG
...
def find_shortest_path(
    current_pos: int, target_pos: int, range_min: int, range_max: int
) -> int:
    """
...
def move_motors_home(
    ...
    try:
        ...
        logger.info("-" * 80)
        logger.info(
            f"{'Motor ID':<10} {'Name':<20} {'Current':<12} {'HOME':<12} {'Min':<8} {'Max':<8}"
        )
        logger.info("-" * 80)

        for motor_name in sorted(robot.bus.motors.keys()):
            try:
                current = robot.bus.read(
                ...
                print(
                    f"{motor_id:<10} {motor_name:<20} {current:<12.0f} {home:<12} {range_min:<8} {range_max:<8}"
                )
            except Exception as e:
        ...
        for motor_name in sorted(robot.bus.motors.keys()):
            try:
                ...
                robot.bus.write(
                    "Goal_Position", motor_name, target_pos, normalize=False
                )
                ...
                print(
                    f"      Current: {current_pos} → Target: {target_pos} (HOME: {home_pos})"
                )
                print(
                    f"      Range: [{range_min}, {range_max}], PID: ({p_coef}, 0, {d_coef})"
                )
            except Exception as e:
        ...
        logger.info("-" * 80)
        logger.info(
            f"{'Motor ID':<10} {'Name':<20} {'Current':<12} {'HOME':<12} {'Status':<15}"
        )
        logger.info("-" * 80)
        ...
        for motor_name in sorted(robot.bus.motors.keys()):
            try:
                ...
                print(
                    f"{motor_id:<10} {motor_name:<20} {current:<12.0f} {home:<12} {status:<15}"
                )
            except Exception as e:
                logger.warning(f"  ✗ {motor_name}: {e}")
                all_ok = False
