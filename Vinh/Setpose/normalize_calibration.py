#!/usr/bin/env python
# FILE NÀY DÙNG ĐỂ CHUẨN HÓA DỮ LIỆU CALIBRATION CỦA MOTOR TỪ BƯỚC (0-4096) SANG ĐỘ [-180, 180]
...
def normalize_calibration(input_file: str, output_file: str = None):
    ...
    print(
        f"{'Motor':<8} {'HOME':<10} {'Min (steps)':<15} {'Min (norm)':<15} {'Max (steps)':<15} {'Max (norm)':<15}"
    )
    print("-" * 100)
    ...
    for motor_id_str in sorted(motor_limits.keys(), key=int):
        ...
        print(
            f"{motor_id:<8} {home_steps:<10} {min_steps:<15} {min_norm:<15.2f} {max_steps:<15} {max_norm:<15.2f}"
        )
    ...
    print("=" * 100)
    print(
        f"{'Motor':<8} {'Min (°)':<15} {'Max (°)':<15} {'Range (°)':<15} {'Usage':<40}"
    )
    print("-" * 100)
